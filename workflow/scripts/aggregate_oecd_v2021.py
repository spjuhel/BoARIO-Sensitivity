...
def aggreg(
    ...
    sec_agg_vec = pd.read_csv(sector_aggregator_path, index_col=1)
    ...
    logger.info(
        "Aggregating from {} to {} sectors".format(
            len(mrio.get_sectors()), len(sec_agg_vec.to_group_id.unique())
        )
    )  # type:ignore
    mrio.aggregate(sector_agg=sec_agg_vec["new sector name"].values)
    mrio.calc_all()
    logger.info("Done")
